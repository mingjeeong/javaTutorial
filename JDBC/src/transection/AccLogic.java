...
public class AccLogic 
{
	...
	public int moveAccount(String sendAcc, String recvAcc, int amount)
	{
		...
		try {
			...
			String sql_1 = "UPDATE account SET amount =amount-? WHERE account_no =?";
			PreparedStatement ps_1 = con.prepareStatement(sql_1);
			ps_1.setInt(1, amount);
			ps_1.setString(2, sendAcc.trim());
			int result_1 = ps_1.executeUpdate();

			String sql_2 = "UPDATE account SET amount =amount+? WHERE account_no =?";
			PreparedStatement ps_2 = con.prepareStatement(sql_2);
			ps_2.setInt(1, amount);
			ps_2.setString(2, recvAcc.trim());
