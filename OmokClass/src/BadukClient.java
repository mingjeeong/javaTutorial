...
class BadukClient extends JPanel implements ActionListener,Runnable {
	...
	// ==================================================
	/*
	 * ClientBadukMain 실행시 도스에서 입력값으로 서버IP와 사용자아이디 지정 인자값이 없는 경우는 127.0.0.1 서버에
	 * guest 아이디 지정
	 */
	BadukClient() {
		this("70.12.115.67", "guest");
	}
	...
	/*
	 * 1.사용자가 입력한 값을 얻어오기 
	 * 2.BadukClientProtocol 생성 클라이언트 ->서버 전송시 사용 객체 
	 * 3.2번에 data와 state지정 
	 * 4.2번 객체를 서버에 전송
	 * 
	 */
	void chatMessage(){
		...
		BadukClientProtocol p = new BadukClientProtocol();
		p.setData(msg);
		p.setState(BadukClientProtocol.Chatting);
		sendInformation(p);
		tfMessage.setText("");
	...
	// =================================================
	/*
	 * 화면을 완성하는 GUI 구성하기 - 화면 관련 멤버 변수의 객체를 생성 - JPanel에 layout에 맞게 붙이기
	 */
	void createGUI() {
		// <- 1
		// West 영역 완성
	...
	// ==================================================
	/*
	 * 소켓클래스로 서버에 연결 1. 소켓클래스 생성 ( serverIP, port번호 ) 2. 생성된 소켓객체의 입력/출력 스트림
	 * 얻어오기
	 */
	void connectServer() {
		try {
	...
	public void run() {
		while(socClient.isConnected()){
			try {
				BadukServerProtocol p = (BadukServerProtocol) inStream.readObject();
	...
	// ----------------------------------------------------
	/*
	 * 서버에 접속후 입력한 아이디를 서버에 전송 1. 클라이언트에서 서버로 전송 데이타 객체 생성 ( BadukClientProtocol
	 * ) 2. 1번 객체에 state는 BadukClientProtocol.SEND_ID로 지정 3. 1번 객체에 data는 입력받은
	 * id로 지정 4. 서버로 전송 ( 소켓의 출력스트림에 write ) : sendInfomation() 이용
	 */
	void sendId() {
	...
	// ---------------------------------------------------
	/*
	 * 넘겨받은 아이디값들을 콤보박스에 추가 1. 넘겨받은 객체를 다시 String[]로 형변환 2. 기존의 콤보박스의 내용들 모두 지움
	...
	// ---------------------------------------------------
	/*
	 * "게임하기" 버튼 클릭시 서버에 전송 1. 클라이언트에서 서버로 전송 데이타 객체 생성 ( BadukClientProtocol )
	 * 2. 1번 객체에 state는 BadukClientProtocol.REQUEST_GAME 값으로 지정 3. 서버로 전송 ( 소켓의
	 * 출력스트림에 write ) : sendInfomation() 이용 4. "게임하기" 버튼을 한번 누르면 비활성화
	 */
	void startGame() {
	...
	// --------------------------------------------------
	/*
	 * 1. 넙겨받은 데이타(BadukServerProtocol의 data임)를 벡터로 변환 2. 벡터의 0번째 요소( 흑돌 게임자의
	 * 아이디 )를 얻어 흑돌게임자 텍스트필드에 지정 3. 벡터의 1번째 요소( 흰돌 게임자의 아이디 )를 얻어 흰돌게임자 텍스트필드에
	 * 지정
	 */
	...
	// --------------------------------------------------
	/*
	 * 게임시작시 초기화 작업 1. 바둑알정보(Rock 클래스)을 저장할 벡터변수(badukRock) 객체 생성 2. 게임이 시작됨을
	 * 지정할 변수(bGamming)을 true로 초기화 3. 승리자 텍스트필드 초기화 4. 바둑판 캔버스 초기화
	 */
	...
	// --------------------------------------------------
	/*
	 * 마우스 클릭한 위치에 대한 바둑알의 정보를 서버에 전송 1. 인자로 넘어온 row와 col 값을 임시벡터 변수에 저장 2. 서버에
	 * 전송 - BadukClientProtocol 객체 생성 - state에
	 * BadukClientProtocol.SET_BADUK_ROCK로 지정 - data에 1번 객체(임시벡터변수) -
	...
	// -----------------------------------------------------
	/*
	 * 서버로부터 놓여진 바둑돌의 위치정보를 받았을때 1. 넘겨받은 객체를 Baduk클래스로 형변환 2. Baduk객체에서 색상을 얻어서
	 * 흑인지 비교후 순서를 백으로 지정 3. 아니면 흑으로 지정 4. Baduk객체를 바둑돌을 저장하는 벡터(badukRock)에 추가
	 * 5. 캔버스를 다시 그리기
	 */
	...
	// ------------------------------------------------
	/*
	 * 게임승리자를 출력하고 모든 화면 초기화 1. 게임진행중을 확인하는 boolean 변수(bGamming)에 false 지정 2.
	 * 넘겨받은 데이타값을 승리자 텍스트필드에 출력 3. "게임하기"버튼을 활성화
	 */
	...
	class MyCanvas extends Canvas {
		...
		MyCanvas() {
			...
			/*
			 * 1. 눌려진 좌표값 얻어오기 2. 1번에서 x값이 바둑판의 몇번째 세로줄인지 지정 ( getXValue() 이용 )
			 * 3. 1번에서 y값이 바둑판의 몇번째 가로줄인지 지정 ( getYValue() 이용 ) 4. 서버에 전송하기 위해
			 * sendSetBaduk() 호출
			 */
