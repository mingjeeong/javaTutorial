...
class Info {
	...
	void eventProc(){
		//이벤트 핸들러 객체 생성
		MyButton btHdlr = new MyButton();
		//이벤트 등록
		bInsert.addActionListener(btHdlr);
		bModify.addActionListener(btHdlr);
		bDelete.addActionListener(btHdlr);
		bShow.addActionListener(btHdlr);
		bExit.addActionListener(btHdlr);
		
		MyText txHdlr = new MyText();
		tfTel.addActionListener(txHdlr);
		
		MyWin winHdlr = new MyWin();
		f.addWindowListener(winHdlr);
		
		MyFocus foHdlr = new MyFocus();
		tfJumin.addFocusListener(foHdlr);
	...
	class MyWin extends WindowAdapter{
		...
		public void windowClosing(WindowEvent e) {
			int result = JOptionPane.showConfirmDialog(null, "정말로 닫으시겠습니까?");
